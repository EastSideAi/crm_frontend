...
CREATE OR REPLACE VIEW finmodel.v_fund_balances WITH (security_invoker='on') AS
 ...
 SELECT period_id, "ведомость", "статус_ведомости", account_id, "счёт", "вид", sort,
    ...
    (((("остаток_с_прошлого" + "поступления") - "расходы_с_фонда") - "ушло_с_этого_счёта") + "пополнение") AS "остаток_на_следующий",
        CASE
            WHEN ((((("остаток_с_прошлого" + "поступления") - "расходы_с_фонда") - "ушло_с_этого_счёта") + "пополнение") < (0)::numeric) THEN 'фонд ушёл в минус — платили больше, чем на нём было'::text
            ELSE ''::text
        END AS "тревога"
   FROM d;

-- Тревога считалась по остатку ДО отчислений этого периода, а отчисления становятся
-- фактом только при закрытии. Из-за этого она горела у четырёх фондов из пяти, все
-- четыре при этом были в плюсе. Сигнал, который звенит всегда, читать перестают,
-- поэтому теперь он звенит на том, что и означает: фонд ушёл в минус по итогу.
...
--   select счёт, тревога from finmodel.v_fund_balances order by sort;
--   ждём пустую тревогу у всех фондов: по итогу периода все в плюсе.
